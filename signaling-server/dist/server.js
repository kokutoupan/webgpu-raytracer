...
const server = http_1.default.createServer((req, res) => {
    ...
    if (url.pathname.startsWith("/admin")) {
        ...
        if (url.pathname === "/admin" || url.pathname === "/admin/") {
            const htmlPath = path_1.default.resolve(__dirname, "admin.html");
            if (fs_1.default.existsSync(htmlPath)) {
                res.writeHead(200, { "Content-Type": "text/html" });
                res.end(fs_1.default.readFileSync(htmlPath));
            }
            else {
                res.writeHead(404);
...
wss.on("connection", (ws, req) => {
    ...
    if (!token || token.length !== SECRET.length) {
        console.log(`Connection rejected: Missing or invalid length token from ${req.socket.remoteAddress}`);
    ...
    const isValid = (0, crypto_1.timingSafeEqual)(sourceBuffer, targetBuffer);
    if (!isValid) {
    ...
    ws.on("message", (message) => {
        ...
        switch (data.type) {
            case "register_host": {
                ...
                if (hostSocket != null && hostSocket.readyState === ws_1.WebSocket.OPEN) {
                    console.warn(`[Refused] Host registration from ${ws.id} refused. Host ${hostSocket.id} is already active.`);
                ...
                for (const [workerId, workerWs] of workers) {
                    if (workerWs.readyState === ws_1.WebSocket.OPEN) {
                        sendTo(ws, {
                            type: "worker_joined",
                            workerId: workerId,
                        });
            ...
            case "register_worker": {
                ...
                if (data.sessionId && data.sessionToken) {
                    ...
                    if (storedToken && storedToken === data.sessionToken) {
                        ...
                        if (oldWs && oldWs !== ws && oldWs.readyState === ws_1.WebSocket.OPEN) {
                            oldWs.close();
                        }
                    }
                    else {
                        console.warn(`Invalid session resumption attempt: ${data.sessionId}`);
                    }
                }
                // Assign/Update ID
                ws.id = finalId;
