...
pub fn load_gltf(
    ...
    for mesh in gltf.meshes() {
        ...
        for primitive in mesh.primitives() {
            ...
            let emissive_col = vec3(emissive[0], emissive[1], emissive[2]);

            // Material Type Selection
            let mut mat_type = crate::scene::material::LAMBERTIAN;
            if metallic > 0.0 {
                mat_type = crate::scene::material::METAL;
            }
            if emissive_col.length_squared() > 1e-4 {
                mat_type = crate::scene::material::LIGHT;
            }

            // Textures
            let mut tex_indices = [-1.0; 4];
            if let Some(info) = pbr.base_color_texture() {
                tex_indices[0] = info.texture().index() as f32;
            }
            if let Some(info) = pbr.metallic_roughness_texture() {
                tex_indices[1] = info.texture().index() as f32;
            }
            if let Some(info) = mat.normal_texture() {
                tex_indices[2] = info.texture().index() as f32;
            }
            if let Some(info) = mat.emissive_texture() {
                tex_indices[3] = info.texture().index() as f32;
            }

            let occl_idx = mat
                .occlusion_texture()
                .map(|i| i.texture().index() as f32)
                .unwrap_or(-1.0);
